...
## B:32 — Enabling bucket-level encryption keys moves the KMS encryption context from the object to the container, so an object-scoped grant condition denies every operation

**Statement.** Object storage offers a bucket-level key optimization that reduces key-service calls by deriving a short-lived data key once per container instead of once per object. Enabling it is presented as a pure cost and throughput change, and it does not alter what is encrypted or who may read it. What it does alter is the ENCRYPTION CONTEXT the storage service presents to the key service: with the optimization off, the context names the individual object; with it on, the context names the CONTAINER. Any key grant or key policy that constrains itself with an encryption-context condition written in the object form therefore stops matching, and every operation that must encrypt or decrypt through that grant is denied. The denial lands on whichever principal depends on the grant — commonly an asynchronous one such as a replication, lifecycle, or archival role — so it does not surface as a user-visible error but as a silently stalled background process, with objects accumulating in a failed state. The condition is not wrong in the ordinary sense: it was correct before the optimization was enabled, and the change that broke it touched a different resource entirely.
