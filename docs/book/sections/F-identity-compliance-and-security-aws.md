...
**Statement.** Services that isolate work per customer, per workspace, or per profile commonly assume a stamped role whose name is derived at runtime from an entity identifier. The caller is granted the family by wildcard, so the grant is written once and correct forever. When the derived role does not exist — provisioning never ran for that entity, ran before the role was added to the template, or the role was removed by a teardown that missed its dependents — the identity service does NOT report that the role is absent. It returns an authorization denial, deliberately, so that callers cannot enumerate which role names exist. The error text then names the caller, the action, and the target role, and reads exactly like a missing grant. Every instinct it triggers is wrong: the policy is inspected and found correct, the wildcard is widened, the trust policy is re-read, and none of it changes anything, because the resource under discussion is not there at all. The defect is per-entity, so it is invisible in aggregate health — most entities work — and it surfaces as an intermittent failure correlated with nothing the caller can see.
