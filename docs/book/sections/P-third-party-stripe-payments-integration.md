...
## P:21 — A provider webhook carries a catalog identifier the local plan map does not contain, so the event fails permanently while the provider keeps retrying it

**Statement.** Billing integrations translate the payment provider's catalog identifier — a price, product, or plan id — into an internal plan through a map held in application code or configuration. That map is maintained by hand and updated when someone remembers, while the provider's catalog is edited in the provider's own dashboard by whoever is running an experiment, a promotion, or a migration. The two drift in one direction only: the catalog gains entries the map does not have. A webhook carrying an unmapped identifier then throws, and because the throw happens during translation — before any idempotency release, ledger write, or entitlement grant — the customer's payment succeeds at the provider while the entitlement it bought is never granted. The provider's retry policy makes this worse rather than better: the same event is redelivered on a schedule for hours or days, each attempt failing identically, so the error rate looks like a systemic outage rather than one unmapped identifier. Because catalog edits happen outside the repository, nothing in the deployment history explains the onset.

**Detect.** Locate every translation from a provider identifier to an internal concept and establish where its map lives and who may edit each side. Then compare the two populations directly: list the provider's active catalog identifiers through its API and diff them against the map's keys — every identifier present at the provider and absent from the map is a future failure with a known trigger. In logs, search for the translation's own error token rather than for generic failures, and pull the concrete identifiers out of it. Confirm the blast radius by checking whether the throw precedes the idempotency release and the entitlement write, which decides whether the customer merely waits or is charged without receiving anything.

**False positives.** Identifiers that are legitimately unmapped because they belong to another product line or a test-mode catalog, where the correct behaviour is an explicit ignore — the ignore must be present in code, not inferred from intent; integrations that resolve the plan from the provider's object at runtime rather than from a local map; and single occurrences during a deliberate catalog cutover where the map landed shortly after, which the deployment history must show.
