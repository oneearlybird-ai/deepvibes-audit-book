...
## R:20 - The vendor's per-conversation success verdict is produced by a model reading the transcript, so a call in which every tool failed and the agent invented the answers is marked successful, and any monitoring built on that field is blind to the failures that matter most

**Statement.** Conversational-AI platforms attach an evaluation to each finished conversation — a
success flag, a rating, a summary — and teams reach for it as the obvious health signal: a
dashboard of green calls, an alarm on the failure rate, a KPI in the weekly report. The verdict is
generated by a model reading the transcript against a rubric, and a transcript is the agent's
own account of the call. When a tool fails, the agent that fails with it says so and the verdict
turns red; when a tool fails and the agent covers — states a price it never looked up, offers
openings it never checked, promises a text that was never sent — the transcript reads as a
competent, complete call and the verdict is green. The flag therefore inverts precisely on the
failure class that costs the most: it catches the honest failure and blesses the fabricated
success. Any reporting or alerting downstream of it inherits the inversion, and because the flag is
the vendor's own and looks authoritative, the inversion is trusted longer than a first-party signal
would be. The same conversation record usually carries the truth elsewhere — the tool-call trail
with per-call error markers, the termination reason — and nothing in the estate joins them.

**Detect.** Take conversations known to have failed — a tool trail with errors, a customer
complaint, a detector hit — and read the vendor's verdict on each. One green verdict on a call
whose tool trail is all errors is the defect. Then find every consumer of the verdict field:
dashboards, alarms, reports, the fields written to the call record. Each is a monitoring path that
cannot see this failure class. Check whether anything in the estate computes its own verdict from
structural facts the model cannot narrate away — tool calls attempted versus succeeded, whether
a booking mutation ran when the agent said it did, whether the caller was told to call back.

**False positives.** A verdict the platform documents as covering conversational quality only,
consumed only for that purpose, alongside a first-party outcome signal built from the tool trail.
