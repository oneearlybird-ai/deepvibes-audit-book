...
## T:39 — The session verifier catches an infrastructure denial while fetching its signing secret and returns "no session", converting a platform fault into a silent per-request logout

**Statement.** Session verification needs a signing secret, fetched at runtime from a secret store and cached briefly. The fetch can fail for two categorically different reasons: the caller presented no credential to verify, or the verifier itself could not reach or was not permitted to read the secret. The first is an authentication outcome; the second is a platform fault. A verifier that wraps the fetch in a catch and returns its "no valid session" sentinel collapses the two, and every caller downstream reads the platform fault as a correct, confident statement that the requester is not signed in. The consequences follow the authentication path rather than the error path: users are redirected to sign in, clients discard their session state, and requests are refused with an ordinary unauthorized status. Nothing raises an error rate, because no request errored; the only trace is a log line at the catch site, which is why these incidents are typically discovered from a log-derived alarm rather than from any user-facing signal. The failure is also self-concealing under caching — while a cached key remains valid the system behaves normally, so the outage begins at an arbitrary later moment when the cache expires, disconnected from the change that caused it.
