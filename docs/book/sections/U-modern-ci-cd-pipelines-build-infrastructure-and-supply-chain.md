...
## U:47 — A build tool that materializes a copy of the source tree into the working directory makes filesystem-walking scanners read the generated copy as source

**Statement.** Wrapper and orchestration tools frequently work by COPYING the unit they are about to build into a cache directory inside the working tree and running the underlying tool from that copy. This is invisible to version control — the directory is ignored — and invisible to any tool that reads the repository through version control. It is entirely visible to scanners that walk the FILESYSTEM: static analyzers, policy scanners, secret scanners, license scanners. Every finding is then reported twice, once against the real source path and once against a generated path that nobody can edit and that will not exist after the next clean. The practical damage falls on baselines: a suppression or accepted-risk entry is keyed to the real path, so the duplicate appears as a NEW finding and blocks the gate, and the blockage arrives immediately after adopting the wrapper, when attention is on the migration rather than the scanner. Attempts to fix it by adding the generated path to the baseline are worse than the defect, since the cache path contains a content hash and a new one appears on every change.

**Detect.** Enumerate every scanner in the gate set and classify each by what it walks — version control or the filesystem. For each filesystem walker, list the generated directories any build wrapper materializes inside the tree and confirm whether the scanner excludes them; tools that skip dot-directories by default may already be safe, and that must be verified per tool rather than assumed from one. Confirm by running the gate with the cache directories PRESENT on disk: a gate that passes only after the evidence is cleaned has not been fixed. Cross-check the baseline for entries whose path contains a cache or hash segment — those are the signature of the wrong fix.

**False positives.** Scanners invoked with an explicit file list derived from version control; wrappers configured to place their cache outside the tree; and repositories where the generated copy is genuinely the artifact under review, such as a vendored dependency directory that ships, which must be scanned and is not a build artifact. The out-of-tree placement only clears the rule where it is configured for every way the wrapper is actually run. A placement exported by the task runner's environment, rather than written into the wrapper's own configuration file, protects only runs that go through the task runner. Any guide, runbook or README that tells people to invoke the wrapper directly brings the in-tree copy straight back. So read every documented invocation, not just the task runner, and confirm each one sets the placement.

## U:48 — A freshness gate that compares timestamps calls a content-preserving relocation stale, because the move changes every date without changing a byte

**Statement.** A deployment-lag or staleness gate answers "is what is published still current?" by comparing the publication time of an artifact against the modification time of its inputs — a commit date, a file date, a directory date. The comparison is sound while inputs change only when their content changes. A repository restructure breaks that assumption: moving a directory rewrites the history date of every file inside it while leaving the bytes identical, so the published artifact's date now precedes its inputs' dates and the gate declares it stale. The build system, which decides by content hash, simultaneously reports nothing to rebuild. The two signals disagree, and the human reading them is asked to reconcile a contradiction that has no correct resolution: publishing again produces an artifact identical to the one already deployed. Left in place the gate is worse than absent, because a check that is red for a reason nobody can act on trains its readers to override it, and the override is what carries into the day the gate is red for a real reason.
